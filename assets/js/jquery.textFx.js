...
(function($){
	function t(t,n,r,i){
		...
		if(s.length){
			$(s).each(function($,t){
				o+='<span class="'+r+($+1)+'">'+t+"</span>"+i});
			t.empty().append(o);}}
		var n={init:function(){
					return(this.each(function(){t($(this),"","char","")}));},
			words:function(){return(this.each(function(){t($(this)," ","word"," ")}));},
			lines:function(){return(this.each(function(){
											var n="eefec303079ad17405c889e092e105b0";
											t($(this).children("br").replaceWith(n).end(),n,"line","");}));}};
			$.fn.lettering=function(t){
					if(t&&n[t]){
						return(n[t].apply(this,[].slice.call(arguments,1)));}
					else if(t==="letters"||!t){
						return(n.init.apply(this,[].slice.call(arguments,0)));}
					$.error("Method "+t+" does not exist on jQuery.lettering");
					return this; }
})(jQuery);
(function ( $ ) {
	$.fn.textFx = function(options) {
		//Default settings for $.textFx();
		var o = $.extend({type: 'fadeIn',iChar: 150,iAnim: 500,direction: 'top'}, options);
		//Execute
		return this.each(function() {
			...
			if (o.type == "fadeIn") {
				...
				letters.each(function() {
					$(this).delay(o.iChar * counter).transition({opacity: 1}, o.iAnim);
					counter += 1;});}
			if (o.type == "slideIn") {
				if (o.direction == "top") {
					letters.css({position: 'relative',top: - letters.position().top - (letters.height() * 2)});}
				if (o.direction == "bottom") {
					letters.css({position: 'relative',top: $(document).height() - letters.position().top + (letters.height() * 2)});}
				if (o.direction == "left") {
					letters.css({position: 'relative',left: - letters.position().left - $(this).width()});}
				if (o.direction == "right") {
					letters.css({position: 'relative',left: letters.position().left + $(this).width()});}
				letters.each(function() {
					$(this).delay(o.iChar * counter).transition({top: 0,left: 0}, o.iAnim);
					counter += 1;});}
	
			if (o.type == "rotate") {						
				letters.css({opacity: 0	});
				letters.each(function() {
					$(this).delay(o.iChar * counter).transition({opacity: 1,rotate: 360}, o.iAnim);
					counter += 1;});}
			
			if (o.type == "scale") {						
				$(this).css({scale: 0});
				$(this).each(function() {
					$(this).delay(o.iChar * counter).transition({scale: 1}, o.iAnim);
					counter += 1;});}
		});
	};
